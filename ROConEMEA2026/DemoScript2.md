...
And we're massively expanding the resource types that get a TruRisk score. That S3 bucket isn't
just listed as 'exists',it has a risk score based on its actual
configuration, its exposure, whether there's sensitive data in it,
...
but a prioritised view of what actually matters.

### 2B: Cloud Posture & DSPM (~2 minutes)

**[VIDEO: Posture dashboard and DSPM workflow]**

Now let's look at posture. I can't stress enough the importance of posture
management, breach after breach, it's the misconfiguration that
opens the door. This is a unified multi-cloud posture view,
...
See this filter for data issues? This is DSPM, Data Security
Posture Management,built directly into TotalCloud. Now, we've
...
But the question you really need answered is: does it actually
matter? Is there sensitive data in that bucket? Because an exposed
bucket with test data in it is a very different risk proposition to
an exposed bucket full of PHI or PCI data.

That's what DSPM does. We're now classifying the data inside your
storage accounts, databases, and container images. PII, PHI, PCI
...
When I click into this storage account, we can see it contains PHI
...
to the Kubernetes API to pull your inventory. For serverless
...
container a hierarchical identity based on where it lives in your
infrastructure: account, cluster, namespace, pod, container. Think
of it as a logical address.
...
the same image get grouped together. It turns what would otherwise
be container chaos into a navigable hierarchy, and it's the
foundation for how we apply TruRisk scoring at every level of that
hierarchy.
...
do when you're ready. But the key message is: you don't have to
start there. The agentless scanning gives you immediate value.
Runtime adds depth.
...
And notice, we're marking which containers are running AI
...
### 3C: Runtime + Exposure Context (~1 minute)

**[VIDEO: Image layer detail with code findings]**

So this is where the runtime and exposure context really changes
the game for prioritisation. A lot of tools can scan an image
statically. But that's only part of the picture.
...
drives prioritisation when you have millions of containers. Not
what could theoretically be exploited, but what's actually running,
actually exposed, and actually risky.
...
containerised and AI attack surface. Not just pre-breach scanning,
but real post-breach detection capability as well.

**Transition:** So we've discovered everything, we've assessed the
risk. Now the question is, how do we actually act on it?

---

## PART 4: Attack Path & Agentic AI Orchestration (~3.5 minutes)
...
there's one thing we've learnt from studying breaches, it's that
they are almost never a single
...
So Steve has built the playbook. The question then is, how does
that actually get executed? And this is where Agent Luther comes in.

Luther is the agent that turns remediation intent into action across
your operational toolchain. Three key things it does.

First, ServiceNow integration. Luther can automatically create
SNOW tickets from prioritised findings. And the ticket arrives with
full context: the attack path, the affected resources, the
recommended remediation, the TruRisk score. Your ops team isn't
starting from scratch, they've got everything they need to act.

Second, auto-merge PRs. Luther leverages QScanner as an MCP
Server to work directly with GitHub, GitLab, or whatever SCM you're
using. It can generate a fix, open a pull request, and if your
policy allows, auto-merge it. Code-level remediation without the
developer needing to context-switch into a security console.
...
language prompt describing what you need. The LLM builds the
workflow, you review it, approve it, and it executes. This is where
those organisation-specific policies and practices I mentioned get
built in.
...
So the full cycle is: discover, assess, validate, remediate via
Luther, and then Val goes back and revalidates to confirm the
exposure is actually closed. That closed-loop verification is
something I think is going to be really important as we move to
...
at the end of the day, remediation happens in code. It happens in
pull requests, in CI/CD pipelines, on developer workstations. And
I think the relationship between security and development teams is
absolutely fundamental to a successful security programme.
...
But here's what ties back to what Luther is doing: QScanner as an
MCP Server. This means developers can interact with Qualys security
data directly from their IDE, VS Code, GitHub, GitLab. They can
check if their image has vulnerabilities, get remediation guidance,
and see exactly what Luther has flagged for them.

When Luther opens that auto-merge PR, the developer sees the full
context, which vulnerability, which image layer, which dependency
introduced it, and the suggested fix. No security console required.
No context switching. Security data landing right in their toolchain,
which as someone with a development background, I can tell you
makes an enormous difference to adoption.

This is the code-to-cloud-to-runtime remediation loop: QScanner
catches it in the pipeline, Luther operationalises the fix, Val
confirms the exposure is closed. End to end.
...
4. **Container Security** with QLP provides full lifecycle
   ...
   real prioritisation
5. **Agentic AI**, Nexa, Steve, Luther, Val, prioritises,
   remediates, operationalises, and validates in a closed loop
...
- **Video 4:** Attack path -> Nexa prioritisation -> Steve remediation playbook -> Luther (SNOW ticket, auto-merge PR, QFlow prompt)
...
- **Runtime + exposure context**, not just what could be exploited, but what's actually running, actually exposed, actually risky
