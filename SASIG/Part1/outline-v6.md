...
| 5 | S1 | The four assumptions every secure SDLC rests on (v4 P1.3) | W/R/D/T introduced as concept - the four trust delegations the discipline rests on. The *only* slide where this frame is the focus. Closing pivot: "with AI in the loop, what does the developer actually know?" Footer pull-quote from Anthropic 2026 Agentic Coding Trends Report - peer-credible reinforcement. |
...
| 7 | S2 | From tool to author | Merged: trajectory (vibe coding, 75% at Google, 25 -> 50 -> 75 in two years) as hero; adoption headcount (4.7M Copilot, 90% DORA, McKinsey, frontier labs) as footer band. Business-case line: 20-40% opex reductions / 12-14pp EBITDA gains for AI-centric orgs (the "why now" the board hears). |
...
| 13 | S2 | The AI toolchain is itself an attack surface (v2 Slide 18) | LiteLLM March 2026; 2026 CVEs in Cursor / Copilot / Claude Code / Windsurf; ~2,000 production MCP servers, none authenticated. Shadow-AI / IP-leakage beat: 3 of 4 CISOs find unsanctioned coding tools running; prompts routinely include proprietary code and credentials sent to vendor LLMs. |
| 14 | S3 | Tooling - what a confidently AI-enabled SDLC looks like (v4 P3.1) | Reframed as the "yes path" - the four gates are the *means* of confident AI adoption, not the destination. Inventory first, then IDE-boundary controls (Spotlighting 50% -> 2% ASR), then fail-closed PR gates, then build-side pinning. Footer band: AI-aware SAST as the new perimeter category - the defenders are evolving too. |
